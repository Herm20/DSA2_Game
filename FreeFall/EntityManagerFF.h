...
namespace Simplex
{
	class Simplex Entity
	{
		...
		static EntityManager* instance;	// singleton pointer
	...
	public:
		...
		Entity* GetEntity(uint index = -1);	// gets the unique id of the entity indexed
		...
		RigidBody* GetRB(uint index = -1);	// returns the rigidbody associated with this entity at this index
		RigidBody* GetRB(String uID);	// returns the rigidbody associated with this entity with this uID
		...
		void UsePhysicsSolver(bool use = true, String uID);	// resolves using physics solver or not in the update
